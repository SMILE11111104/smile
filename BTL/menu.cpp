...
void tim_tt(Ds *&ds){
    ...
    if (x == 1){
        cout << "\n\tSo cmt/cccd bac sy can tim kiem: "; getline(cin, cmt);
        ps =  Tim_kiem(ds, "SB" + cmt, x);
        if (ps == NULL) {
            cout << "\n\tKhong ton tai bac sy co so cmt/cccd da nhap!!!" << endl;
            return;
        }
        ...
        cout << "\n\tMa benh nhan can tim kiem: "; getline(cin, mbn);
        ps = Tim_kiem(ds, mbn, x);
